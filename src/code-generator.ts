...
export class CodeGenerator {
    ...
    async generateWrapper(
        ...
        // Without the defining module's name the wrapper cannot import the
        // function, so the generated project is guaranteed to fail with an
        // inscrutable "Variable not in scope" — fail fast and clearly instead.
        if (!func.moduleName) {
            throw new Error(
                `Could not determine the module name of ${func.filePath}. ` +
                'Add an explicit "module Foo.Bar where" header to the file.'
            );
        }

        // Without a type signature the Synthesize annotation's port list would
        // be generated blind and disagree with the function's real arity —
        // Clash would reject it with a confusing error. Fail fast instead.
        if (!func.typeSignature) {
            throw new Error(
                `No type signature detected for ${func.name}. ` +
                'HLS may still be indexing — save the file, wait a moment, and try again.'
            );
        }
    ...
    async ensureSynthProject(
        ...
        if (cabalProject) {
            ...
            if (userCabalProjectFile) {
                ...
                cabalProjectContent = [
                    '-- Auto-generated by clash-toolkit. Do not edit.',
    ...
    private generateSynthCabalFile(
        ...
        // Quote entries containing whitespace — cabal's list parsing breaks
        // on unquoted paths with spaces (e.g. a workspace under "My Project").
        const srcDirs = ['src', ...(extraSourceDirs || [])]
            .map(d => (/\s/.test(d) ? `"${d}"` : d))
            .join(', ');
    ...
    private generateWrapperContent(func: FunctionInfo, moduleName: string): string {
        // generateWrapper fails fast when either of these is missing.
        const originalModule = func.moduleName!;
        const functionName = func.name;
        const typeSignature = func.typeSignature!;
        ...
        lines.push('{-# OPTIONS_GHC -Wno-missing-signatures #-}');
        lines.push('');
        ...
        lines.push('-- This module was auto-generated by clash-toolkit extension');
        lines.push(`-- Original type: ${typeSignature}`);
        lines.push('');

        // topEntity definition — no explicit type signature; the type is
        // inherited from the re-exported binding via GHC inference.
    ...
    /**
     * Strip `forall ….` quantifiers and top-level constraint contexts
     * (`KnownNat n => …`) from a type signature, leaving only the actual
     * argument/result chain. Contexts are not arguments, so counting them
     * as ports would make t_inputs disagree with the function's arity.
     */
    private stripContext(type: string): string {
        let t = type.trim();
        t = t.replace(/^forall\b[^.]*\.\s*/, '');
        let depth = 0;
        for (let i = 0; i < t.length - 1; i++) {
            const ch = t[i];
            if (ch === '(' || ch === '[') { depth++; }
            else if (ch === ')' || ch === ']') { depth--; }
            else if (depth === 0 && ch === '=' && t[i + 1] === '>') {
                // Multiple contexts (`C a => D b => t`) strip recursively.
                return this.stripContext(t.slice(i + 2));
            }
        }
        return t;
    }
    ...
    /**
     * Split a type on top-level `->` arrows only — arrows nested inside
     * parentheses or brackets (function-typed arguments like
     * `(Signal dom a -> Signal dom b)`) belong to one argument, not two.
     */
    private splitTopLevelArrows(type: string): string[] {
        const parts: string[] = [];
        let depth = 0;
        let current = '';
    ...
    private generatePortNames(typeSignature: string): { inputs: PortAnnotation[], output: string } {
        // Split on top-level -> to get the argument types, ignoring the
        // constraint context and arrows nested inside parens/brackets.
        const parts = this.splitTopLevelArrows(this.stripContext(typeSignature));
        ...
        // De-duplicate port names — e.g. a dual-clock function would get two
        // "CLK" ports, and duplicate HDL port names are rejected by Clash.
        const seen = new Map<string, number>();
        for (const port of inputs) {
            const count = seen.get(port.name) ?? 0;
            seen.set(port.name, count + 1);
            if (count > 0) {
                port.name = `${port.name}${count + 1}`;
            }
        }
     ...
     * Characters legal in Haskell identifiers but awkward elsewhere
     * (primes, unicode) are replaced so the module name is also a safe
     * file name and cabal exposed-module.
     */
    private generateModuleName(functionName: string, prefix: string): string {
        ...
        return `${prefix}${capitalizedName}`.replace(/[^A-Za-z0-9_]/g, '_');
    }
    ...
    private generateSynthName(functionName: string): string {
        // Convert camelCase to snake_case and lowercase, then replace
        // anything that is not a legal Verilog identifier character —
        // primes (`counter'`) and unicode are legal in Haskell names but
        // rejected by Clash as HDL module names.
        return functionName
            .replace(/([A-Z])/g, '_$1')
            .toLowerCase()
